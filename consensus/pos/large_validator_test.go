	...
	t.Run("100 validators with valid signatures", func(t *testing.T) {
		...
		for i := 0; i < 100; i++ {
			...
			if err == nil {
				...
				t.Errorf("Valid signature rejected for validator-%d: %v", i, err)
			}
		}
...
/*
// NOTE: Uncomment after integrating consensus_signature_fix.go
func BenchmarkSignatureVerification100Validators(b *testing.B) {
	mockState, keys := createValidatorsWithKeys(100, 1000)
	ce := &ConsensusEngine{
		worldState: mockState,
		chainCache: NewChainCache(),
	...
	blockHash := "bench-signed-block"

	// Pre-create 100 signed attestations
	attestations := make([]*Attestation, 100)
	for i := 0; i < 100; i++ {
		address := fmt.Sprintf("validator-%d", i)
		attestations[i] = createSignedAttestation(address, keys[i], blockHash, 1, 32, 100)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = ce.verifyAttestationSignature(attestations[i%100])
	}
}
*/
