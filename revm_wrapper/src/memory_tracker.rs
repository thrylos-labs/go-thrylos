...
impl MemoryTracker {
    ...
    pub fn track_error_message(&self, ptr: *mut c_char) {
        ...
        if set.contains(&addr) {
            eprintln!("⚠️ WARNING: Error message at {:p} already tracked (possible duplicate allocation)", ptr);
        }
    ...
    pub fn track_return_data(&self, ptr: *const u8, len: usize) {
        ...
        if set.contains(&addr) {
            eprintln!("⚠️ WARNING: Return data at {:p} already tracked (possible duplicate allocation)", ptr);
        }
    ...
    pub fn untrack_error_message(&self, ptr: *mut c_char) -> bool {
        ...
        if !set.contains(&addr) {
            ...
            eprintln!("⚠️ WARNING: Attempting to free untracked error message at {:p}", ptr);
            return false;
        }
        ...
        if removed {
            ...
            eprintln!("⚠️ WARNING: Double-free detected for error message at {:p}", ptr);
        }
    ...
    pub fn untrack_return_data(&self, ptr: *const u8) -> bool {
        ...
        if !set.contains(&addr) {
            // Pointer not tracked - either already freed or never allocated by us
            self.invalid_free_attempts.fetch_add(1, Ordering::Relaxed);
            eprintln!("⚠️ WARNING: Attempting to free untracked return data at {:p}", ptr);
            return false;
        }
        ...
        if removed {
            ...
            // This shouldn't happen since we just checked contains()
            self.double_free_attempts.fetch_add(1, Ordering::Relaxed);
            eprintln!("⚠️ WARNING: Double-free detected for return data at {:p}", ptr);
        }
    ...
    pub fn report(&self) {
        ...
        eprintln!("╠════════════════════════════════════════════════════════════════╣");
        eprintln!("║ Total Allocations:          {:>10}                      ║", stats.total_allocated);
        eprintln!("║ Total Frees:                {:>10}                      ║", stats.total_freed);
        eprintln!("║ Potential Leaks:            {:>10}                      ║", stats.potential_leaks);
        eprintln!("╠════════════════════════════════════════════════════════════════╣");
        eprintln!("║ Error Messages Allocated:   {:>10}                      ║", stats.error_messages_allocated);
        eprintln!("║ Current Error Messages:     {:>10}                      ║", stats.current_error_messages);
        eprintln!("╠════════════════════════════════════════════════════════════════╣");
        eprintln!("║ Return Data Allocated:      {:>10}                      ║", stats.return_data_allocated);
        eprintln!("║ Current Return Data:        {:>10}                      ║", stats.current_return_data);
        eprintln!("╠════════════════════════════════════════════════════════════════╣");
        eprintln!("║ Double-Free Attempts:       {:>10}                      ║", stats.double_free_attempts);
        eprintln!("║ Invalid Free Attempts:      {:>10}                      ║", stats.invalid_free_attempts);
        eprintln!("╚════════════════════════════════════════════════════════════════╝");
        
        if stats.potential_leaks > 0 {
            eprintln!("⚠️  WARNING: {} potential memory leak(s) detected!", stats.potential_leaks);
        } else {
            eprintln!("✅ No memory leaks detected");
        }
        
        if stats.double_free_attempts > 0 {
            eprintln!("⚠️  WARNING: {} double-free attempt(s) prevented!", stats.double_free_attempts);
        }
        
        if stats.invalid_free_attempts > 0 {
            eprintln!("⚠️  WARNING: {} invalid free attempt(s) detected!", stats.invalid_free_attempts);
        }
    }
